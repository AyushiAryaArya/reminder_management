...
app.config['MYSQL_PASSWORD'] = 'aditya081'
...
class Registration(Form):
	...
	reg_password = PasswordField('Password',[validators.Length(min=5,max=10), validators.DataRequired(), validators.EqualTo('reg_confirm', message='Password do not match')])
...
def register():
	...
	if request.method == 'POST' and form.validate():
		...
		cur.execute('INSERT INTO registration(NAME,EMAIL,PASSWORD) VALUES (%s,%s,%s)',(reg_name,reg_email,reg_password))
...
def login():
	if request.method == 'POST':
		...
		cur = mysql.connection.cursor()
		result = cur.execute('SELECT * FROM registration WHERE EMAIL=%s',[log_email])
